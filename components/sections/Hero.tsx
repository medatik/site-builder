...
/**
 * Hero. Works for any business — all copy comes from props. Three layouts:
 * `split` (copy + media), `centered`, and `slider` (a full-bleed rotating
 * banner, delegated to `HeroSlider`). A themed CSS backdrop provides the
 * distinctive per-client atmosphere without any niche-specific code.
 *
 * `split` and `centered` stay SERVER components — the slider is the only one
 * that ships JS, and only when a config asks for it.
 */
export function Hero({
  ...
  if (layout === "slider") {
    // Fall back to a single slide built from the top-level copy, so switching a
    // config to `slider` before writing `slides` degrades instead of breaking.
    const resolved =
      slides && slides.length > 0
        ? slides
        : [{ eyebrow, title, highlight, subtitle, primaryCta, secondaryCta, media }];
    return <HeroSlider slides={resolved} autoplayMs={autoplayMs} labels={sliderLabels} id={id} />;
  }
  ...
  const bulletList = bullets && bullets.length > 0 && (
    <ul className={cn("flex flex-col gap-2.5", isCentered && "mx-auto max-w-md text-start")}>
      {bullets.map((b) => (
  ...
  return (
    // Hero builds its own <section> (backdrop layers) instead of the shared
    // Section shell, so it must apply the anchor id itself.
    <section id={id ?? "hero"} className="relative isolate overflow-hidden bg-background pt-[calc(var(--section-py)*0.85)] pb-[var(--section-py)]">
